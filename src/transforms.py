...
class Rotate(object):
    def __init__(self, batch_size, theta_range_1, theta_range_2, fixed_thetas):
        self.count = 0
    ...
    def __call__(self, sample):
        ...
        if isinstance(sample, tuple):
            x_t, x_next = sample
        if self.count % self.batch_size == 0 or isinstance(self.fixed_thetas, np.ndarray):
    ...
    def _generate_angles(self):
        if isinstance(self.fixed_thetas, np.ndarray):
            self.theta_1 = self.fixed_thetas[self.gen_idx]
            print("NEW ANGLES", self.theta_1, self.count, self.gen_idx)
        else:
    ...
    def save_params(self, prepro_params, gen_idx, batch_start_idx, batch_end_idx):
        print("SAVE", gen_idx, batch_start_idx, batch_end_idx)
        theta_diff = self.theta_2 - self.theta_1
        prepro_params["theta_1"][gen_idx][batch_start_idx:batch_end_idx] = self.theta_1
        prepro_params["theta_diff"][gen_idx][batch_start_idx:batch_end_idx] = theta_diff
        self.gen_idx = gen_idx+1
